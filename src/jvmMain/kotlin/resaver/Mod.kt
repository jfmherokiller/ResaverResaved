...
class Mod(game: Game?, dir: Path) : Serializable {
    ...
    companion object:KLoggable {
        ...
        val GLOB_CREATIONCLUB: PathMatcher = FS.getPathMatcher("glob:**\\cc*.{esm,esp,esl,bsa,ba2}")
        val GLOB_INTEREST: PathMatcher = FS.getPathMatcher("glob:**.{esm,esp,esl,bsa,ba2}")
        val GLOB_PLUGIN: PathMatcher = FS.getPathMatcher("glob:**.{esm,esp,esl}")
        val GLOB_ARCHIVE: PathMatcher = FS.getPathMatcher("glob:**.{bsa,ba2}")
        val GLOB_SCRIPT: PathMatcher = FS.getPathMatcher("glob:**.pex")
        val GLOB_STRINGS: PathMatcher = FS.getPathMatcher("glob:**.{strings,ilstrings,dlstrings}")
        val GLOB_ALL: PathMatcher = FS.getPathMatcher("glob:**.{esm,esp,esl,bsa,ba2,pex,strings,ilstrings,dlstrings}")
        val GLOB_EXE: PathMatcher = FS.getPathMatcher("glob:{skyrim.exe,skyrimse.exe,fallout4.exe}")
