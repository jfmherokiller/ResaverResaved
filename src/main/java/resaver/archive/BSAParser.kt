...
import resaver.archive.BSAFileData.getData
...
class BSAParser(path: Path, channel: FileChannel) : ArchiveParser(path, channel) {
    ...
    private fun getNames(channel: FileChannel): Supplier<String> {
        val FILENAMES_OFFSET = (HEADER!!.FOLDER_OFFSET
                + HEADER!!.FOLDER_COUNT * BSAFolderRecord.SIZE + HEADER!!.TOTAL_FOLDERNAME_LENGTH + HEADER!!.FOLDER_COUNT
                + HEADER!!.FILE_COUNT * BSAFileRecord.SIZE).toLong()
    ...
    override fun getFiles(dir: Path, matcher: PathMatcher): Map<Path, Optional<ByteBuffer>> {
        return FOLDERRECORDS!!.stream()
            .filter { block: BSAFolderRecord -> dir == block.PATH }
            ...
            .collect(
                Collectors.toMap(
                    ...
                    { record: BSAFileRecord? -> getData(super.CHANNEL, record!!, HEADER!!) })
            )
    }
    ...
    override fun getFilenames(dir: Path?, matcher: PathMatcher?): Map<Path?, Path?>? {
        return FOLDERRECORDS!!.stream()
            ...
            .collect(
                Collectors.toMap(
                    Function { record: BSAFileRecord -> super.PATH.fileName.resolve(record.path!!) },
                    BSAFileRecord::path
                )
            )
    }
    ...
    var FOLDERRECORDS: MutableList<BSAFolderRecord>? = null
