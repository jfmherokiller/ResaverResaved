...
import resaver.Game
import resaver.ess.CompressionType.Companion.read
...
class Header(input: ByteBuffer, path: Path?) : Element {
    ...
    fun setCompression(newCompressionType: CompressionType?) {
        when (GAME) {
            Game.SKYRIM_SE, Game.SKYRIM_SW, Game.SKYRIM_VR -> if (newCompressionType != null) {
                compression = newCompressionType
                return
            } else {
                throw IllegalArgumentException("The compression type must not be null.")
            }
            Game.SKYRIM_LE, Game.FALLOUT4, Game.FALLOUT_VR -> throw IllegalArgumentException("Compression not supported.")
            else -> throw IllegalArgumentException("Compression not supported.")
        }
    }

    val MAGIC: ByteArray = when (val PREFIX = BufferUtil.readSizedString(input.slice(), 4, false)) {
        "TES4", "FO4_" -> {
            ByteArray(12)
        }
        "TESV" -> {
    ...
    init {
        ...
        when (val MAGICSTRING = String(MAGIC).toUpperCase()) {
            "TESV_SAVEGAME" -> GAME = if (VERSION <= 9 && Game.SKYRIM_LE.testFilename(path)) {
                Game.SKYRIM_LE
            } else if (VERSION >= 12 && Game.SKYRIM_SE.testFilename(path)) {
                Game.SKYRIM_SE
            } else if (VERSION >= 12 && Game.SKYRIM_SW.testFilename(path)) {
                Game.SKYRIM_SW
            } else {
                throw IllegalArgumentException("Unknown version of Skyrim: $VERSION")
            }
            "FO4_SAVEGAME" -> if (11 <= VERSION && Game.FALLOUT4.testFilename(path)) {
                GAME = Game.FALLOUT4
            } else {
        ...
        compression = if (GAME == Game.SKYRIM_SE) read(input) else null
        ...
        BYPP = when (GAME) {
            Game.SKYRIM_LE -> 3
            Game.FALLOUT4, Game.SKYRIM_SE -> 4
