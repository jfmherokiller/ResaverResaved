...
class FragmentTask(input: ByteBuffer, unknown3: Byte, context: PapyrusContext) : PapyrusElement, Linkable {
    ...
    class QuestStage(input: ByteBuffer, context: PapyrusContext) : FragmentData {
        ...
        override fun toHTML(target: Element): String {
            return if (QUEST == null) {
                "$QUESTID stage=$STAGE $FLAGS$UNKNOWN_4BYTES"
            } else {
                "${QUEST.toHTML(target)} stage=$STAGE $FLAGS$UNKNOWN_4BYTES"
            }
        }
        ...
        override fun toString(): String {
            return if (QUEST == null) {
                ...
                "$QUEST stage=$STAGE $FLAGS$UNKNOWN_4BYTES"
            }
        }
        ...
        init {
            ...
            UNKNOWN_4BYTES = if (context.game.isFO4) {
                input.int
            } else {
                null
            }
    ...
    class ScenePhaseResults(input: ByteBuffer, context: PapyrusContext) : FragmentData {
        ...
        override fun toHTML(target: Element): String {
            return if (QUEST == null) {
                "$QUESTID stage=$INT$UNKNOWN_4BYTES"
            } else {
                "${QUEST.toHTML(target)} stage=$INT$UNKNOWN_4BYTES"
            }
        }
        ...
        override fun toString(): String {
            return if (QUEST == null) {
                ...
                "$QUEST stage=$INT$UNKNOWN_4BYTES"
            }
        }
    ...
    class SceneResults(input: ByteBuffer, context: PapyrusContext) : FragmentData {
        ...
        override fun toHTML(target: Element): String {
            return if (QUEST == null) {
                "$QUESTID stage=$UNKNOWN_4BYTES"
            } else {
                "${QUEST.toHTML(target)} stage=$UNKNOWN_4BYTES"
            }
        }
    ...
    class TerminalRunResults(input: ByteBuffer, context: PapyrusContext) : FragmentData {
        ...
        override fun toHTML(target: Element): String {
            return if (FORM == null) {
                "${REFID.toHTML(target)} $TSTRING${String.format(" %08x %02x", INT, BYTE)}"
            } else {
                "${FORM.toHTML(target)} $TSTRING${String.format(" %08x %02x", INT, BYTE)}"
            }
        }

        override fun toString(): String {
            return if (FORM == null) {
                "$REFID $TSTRING${String.format(" %08x %02x", INT, BYTE)}"
            } else {
                "$FORM $TSTRING${String.format(" %08x %02x", INT, BYTE)}"
            }
        }
    ...
    init {
        try {
            ...
            VARIABLE = if (unknown3.toInt() == 3 || unknown3.toInt() == 2) {
