...
internal class DebugFunction(input: ByteBuffer, strings: StringTable) {
    ...
    fun write(output: ByteBuffer) {
        ...
        INSTRUCTIONS.forEach(Consumer { instr: Int -> output.putShort(instr.toShort()) })
    }
    ...
    val fullName: IString
        get() = IString.format("%s.%s", OBJECTNAME, FUNCNAME)
    ...
    override fun toString(): String {
        ...
        INSTRUCTIONS.forEach(Consumer { instr: Int? -> buf.append(String.format("%04x ", instr)) })
        return buf.toString()
    }
    ...
    init {
        ...
        INSTRUCTIONS = ArrayList(instructionCount)
